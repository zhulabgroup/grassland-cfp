...
gg_biogeography <- plot_biogeography (dat_occ)
gg_biogeography
```
...
Comparing using different climate data source
```{r, eval = F}
...
```
```{r}
dat_trait_chelsa <- read_individual_trait(path_trait = path_trait_chelsa)
dat_trait_prism <- read_individual_trait(path_trait = path_trait_prism)
dat_trait_terraclim <- read_individual_trait(path_trait = path_trait_terraclim)
gg_climate_source <- plot_climate_source(dat_trait_chelsa, dat_trait_prism, dat_trait_terraclim) 
```

Comparing using bioclim variables
```{r, eval = F}
path_trait_bioclim <- calc_individual_trait(dat_occ, dat_clim, occ_source = "gbif", clim_source = "chelsa", allbioclim = T)
```
```{r}
dat_trait_bioclim <- read_individual_trait(path_trait = path_trait_bioclim)
...
Comparing using different summary statistics for SNC
```{r}
...
```{r, eval = F}
save_figure(out = list(biogeography = gg_biogeography,
