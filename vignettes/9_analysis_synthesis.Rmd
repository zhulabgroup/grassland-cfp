...
Calculate the trend in abundance of species over time at the observational sites or under warming treatment in the experiment.
```{r, eval=F}
dat_gainloss <- calc_species_gainloss(dat_community = dat_community, dat_niche = dat_niche)
usethis::use_data(dat_gainloss, overwrite = T)
```

```{r, eval = F}
gg_gainloss_elkhorn1 <- plot_species_gainloss_obs(
  obs_tbl = dat_gainloss$obs %>%
    mutate(
      change = "no clear change",
      complete_change = NA
    ),
  dat_niche, onesite = "elkhorn"
)$circle
gg_gainloss_elkhorn2 <- plot_species_gainloss_obs(
  obs_tbl = dat_gainloss$obs %>%
    mutate(
      change = if_else(change == "increase", "increase", "no clear change"),
      complete_change = NA
    ),
  dat_niche, onesite = "elkhorn"
)$circle
gg_gainloss_elkhorn3 <- plot_species_gainloss_obs(
  obs_tbl = dat_gainloss$obs %>%
    mutate(
      change = if_else(change == "increase", "increase", "no clear change"),
      complete_change = if_else(complete_change == "new", "new", NA)
    ),
  dat_niche, onesite = "elkhorn"
)$circle
gg_gainloss_elkhorn4 <- plot_species_gainloss_obs(
  obs_tbl = dat_gainloss$obs %>%
    mutate(complete_change = if_else(complete_change == "new", "new", NA)),
  dat_niche, onesite = "elkhorn"
)$circle
gg_gainloss_elkhorn5 <- plot_species_gainloss_obs(obs_tbl = dat_gainloss$obs, dat_niche, onesite = "elkhorn")$circle
```

Visualize the change in abundance in the climate niche space.
```{r}
gg_species_change <- plot_species_gainloss(dat_niche = dat_niche, dat_gainloss = dat_gainloss)
gg_species_change$combined
```

Sample size in observations:
```{r}
dat_gainloss$obs %>%
  group_by(change) %>%
  summarise(n = n())
...
```{r, eval = F}
gg_species_change_obs_2row <- plot_species_gainloss_obs(obs_tbl = dat_gainloss$obs, dat_niche, nrow = 2)
path_animate_gainloss <- plot_species_gainloss_animate(dat_community, dat_gainloss, dat_niche)
```
...
There were no consistent changes in the evenness of communities.

## Synthesize community shifts in observation and experiments

```{r, eval = F}
dat_shift <- calc_community_shift(dat_index = dat_index)
usethis::use_data(dat_shift, overwrite = T)
```
...
```
Due to the high correlation of species temperature and precipitation niches, CTI and CPI shifts at the community level appear highly coupled.

```{r, eval = F}
gg_community_shift <- plot_community_shift(dat_shift, dat_niche, short = T)
gg_community_shift$combined

ggsave(
  plot = gg_community_shift$combined,
  filename = "alldata/output/figures/main/fig-main-shift-short.pdf",
  width = 9,
  height = 4.5,
  device = pdf
)
...
save_figure(out = list(
  ...
  # gainloss_elkhorn1 = gg_gainloss_elkhorn1,
  # gainloss_elkhorn2 = gg_gainloss_elkhorn2,
  # gainloss_elkhorn3 = gg_gainloss_elkhorn3,
  # gainloss_elkhorn4 = gg_gainloss_elkhorn4,
  # gainloss_elkhorn5 = gg_gainloss_elkhorn5,
  species_change_obs_2row = gg_species_change_obs_2row$circle,
  species_change_obs_2row_name = gg_species_change_obs_2row$circle_detail,
  species_change_exp_small = gg_species_change$exp$circle,
  species_change_obs_box = gg_species_change$obs$box,
  species_change_exp_box = gg_species_change$exp$box,
  rank_abund_obs = gg_rank_abundance$obs,
  rank_abund_exp = gg_rank_abundance$exp,
  community_shift = gg_community_shift$combined,
  community_shift_pt1 = gg_community_shift$niche + gg_community_shift$compare + plot_annotation(tag_levels = "A"),
  community_shift_pt2 = gg_community_shift$obs + plot_annotation(tag_levels = list("C")),
  community_shift_pt3 = gg_community_shift$exp + plot_annotation(tag_levels = list("D"))
))

ggsave(
  plot = gg_species_change$combined,
  filename = "alldata/output/figures/main/fig-main-gainloss.pdf",
  width = 11,
  height = 11.5,
  device = pdf
)
...
ggsave(
  plot = gg_community_shift$combined,
  filename = "alldata/output/figures/main/fig-main-shift.pdf",
  width = 9,
  height = 9 * 1.618,
  device = pdf
)
