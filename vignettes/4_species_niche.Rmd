...
ls_sp <- spp_tbl %>%
  pull(consolidated_name) %>%
  unique() %>%
  sort()
```

We identified `r ls_sp %>% length()` consolidated species from the community data.
...
usethis::use_data(dat_occ, overwrite = T)
```
...
usethis::use_data(dat_trait, overwrite = T)
```
...
bind_rows(
  dat_community$obs %>%
    ...
    left_join(
      dat_community$obs %>%
        group_by(site) %>%
        summarise(abund_total = sum(abund), .groups = "drop"),
      by = "site"
    ) %>%
    mutate(percentage = abund %>% `/`(abund_total) %>% `*`(100) %>% round() %>% str_c("%")) %>%
    select(-abund, -abund_total),
  dat_community$exp %>%
    filter(str_detect(species, "DUMMY")) %>%
    group_by(site, species) %>%
    summarise(abund = sum(abund), .groups = "drop") %>%
...
usethis::use_data(dat_niche, overwrite = T)
```
...
```{r, eval = F}
path_occ_thin <- calc_thin_occ(dat_occ = dat_occ$gbif, dat_trait = dat_trait, option = "spatial", num_cores = 4) # slow
```
...
```{r}
dat_trait_early <- read_individual_trait(path_trait = "alldata/intermediate/climate-niche/gbif-chelsa-early.rds")
```
...
save_figure(out = list(
  ...
  niche_pt1 = gg_niche$occ_geo + gg_niche$ind_trait,
  niche_pt2 = gg_niche$sp_niche,
...
ggsave(
  ...
  filename = "alldata/output/figures/main/fig-main-niche.pdf",
  width = 10,
  height = 8,
  device = pdf
)
