...
```{r}
read_csv("alldata/input/community/species/misspelling.csv") %>%
  head(20) %>%
  knitr::kable()
```
...
usethis::use_data(dat_community, overwrite = T)
```
...
```{r}
read_csv("alldata/input/community/species/consolidation.csv") %>%
  head(20) %>%
  knitr::kable()
```
