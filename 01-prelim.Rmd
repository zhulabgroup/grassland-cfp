...
```{r}
cci_dat <- read_csv("data/archives/CAGrasslandCommunityChange-master/Analysis/CTI Data/AllSite_CTI_08_2020.csv") %>%
  select(-`...1`) %>%
  filter(
...
cci_jrgce_avg %>%
  filter(trt %in% c("____", "T___", "_P__")) %>%
...
cci_jrgce_avg %>%
  filter(trt %in% c("__C_", "T_C_", "_PC_")) %>%
  ggplot(aes(pin_tmp, pin_ppt, col = yr)) +
  geom_path() +
  facet_wrap(~trt) +
  labs(
...
cci_jrgce_avg %>%
  filter(trt %in% c("___N", "T__N", "_P_N")) %>%
  ggplot(aes(pin_tmp, pin_ppt, col = yr)) +
  geom_path() +
  facet_wrap(~trt) +
  labs(
