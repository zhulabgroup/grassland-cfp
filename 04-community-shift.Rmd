...
```{r}
niche_tbl <- read_rds("data/occurrence/niche-estimates-cfp.rds")
```
...
exp_com_tbl <- read_rds("data/community/all-experimental-data.rds") %>%
  ...
  summarize(
    ...
    tmean_occ_upr_com_mean = sum(abund * tmean_occ_upr) / sum(abund),
    ppt_com_mean = sum(abund * ppt_occ_mean) / sum(abund),
    ppt_com_var = sum(abund * ppt_occ_mean^2) / sum(abund) - ppt_com_mean^2,
    ppt_occ_lwr_com_mean = sum(abund * ppt_occ_lwr) / sum(abund),
...
Community-level mean upper temp niche limit in warming vs. ambient plots.

```{r, fig.cap="Compare mean(upper temp) in ambient vs. elevated warming treatments over years"}
exp_com_tbl %>%
  filter(
    site == "JRGCE",
    treat %in% c("____", "T___")
  ) %>%
  ggplot(aes(year, tmean_occ_upr_com_mean, col = treat, group = interaction(treat, year))) +
  geom_boxplot() +
  labs(
    x = "Year", y = "Community average upper temperature (degC)",
    color = "Warming treatment"
  ) +
  scale_color_manual(values = c("black", "red"))
```

Community-level mean CTI in all 64 warming vs. ambient plots (lumping other P, C, N treatments together).
...
Community-level mean lower precip niche limit in watered vs. ambient plots.
...
exp_com_tbl %>%
  ...
  ggplot(aes(year, ppt_occ_lwr_com_mean, col = treat, group = interaction(treat, year))) +
  geom_boxplot() +
  labs(
    x = "Year", y = "Community average lower precipitation (mm)",
    color = "Watering treatment"
  ) +
  scale_color_manual(values = c("black", "blue"))
```
