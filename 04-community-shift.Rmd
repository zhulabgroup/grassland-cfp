...
```{r}
exp_com_tbl <- expdat_tbl %>%
  inner_join(niche_tbl, by = "species") %>%
  group_by(site, year, plot, treat) %>%
  summarize(
    tmean_com_mean = sum(abund * tmean_mean) / sum(abund),
    ppt_com_mean = sum(abund * ppt_mean) / sum(abund),
    vpdmax_com_mean = sum(abund * vpdmax_mean) / sum(abund)
  )
```
