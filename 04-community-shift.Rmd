...
expcom_tbl <- expdat_tbl %>%
  ...
  summarize(
    tmp_com = sum(abund * tmp_mean) / sum(abund),
...
expcom_tbl %>%
  ...
  ggplot(aes(year, tmp_com, col = treat, group = interaction(treat, year))) +
  geom_boxplot() +
  labs(
...
expcom_tbl %>%
  ...
  ggplot(aes(tmp_com, ppt_com, group = interaction(treat, year))) +
...
expcom_tbl %>%
  ...
  group_by(year, treat) %>%
  summarize(tmp_com = median(tmp_com), ppt_com = median(ppt_com)) %>%
  # mutate(year = as.factor(year)) %>%
  ggplot(aes(tmp_com, ppt_com, col = year)) +
  geom_path() +
  facet_wrap(~treat) +
  labs(
    x = "Community Temperature Index (CTI, degC)",
