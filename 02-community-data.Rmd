...
```{r}
rm(list = c("pin_tbl", "spp_tbl", "trt_tbl"))
rm(jrgce_data)
```
...
```{r}
rm(list = c("cover_2015_tbl", "cover_2016_tbl", "cover_2017_tbl", "cover_2018_tbl", "cover_2019_tbl", "cover_2021_tbl", "cover_long_tbl", "plot_tbl"))
rm(mclexp_data)
```
...
```{r}
remove(list = c("Angelo", "Angelo_spp"))
remove(angelo_data)
```
...
```{r}
remove(list = c("carrizo", "carrizo_extras", "carrizo_forms", "carrizo_spp"))
remove(carrizo_data)
```
...
```{r}
remove(list = c("elk_counts", "elk_counts_clean", "elk_counts_df", "elk_guilds", "elkspp", "out"))
remove(list = c("filename", "i", "skips", "year"))
remove(elkhorn_data)
```
...
```{r}
remove(list = c("Jasper", "Jasper_species", "jasper_spp_check", "Jasper_Veg", "JR_spp"))
remove(jasper_data)
```
...
```{r}
remove(list = c("McL_Sites", "McL_spp", "McLAnn"))
remove(mclann_data)
```
...
```{r}
remove(list = c("McL_Sites", "McL_spp", "McLSerp"))
remove(mclserp_data)
```
...
```{r}
remove(list = c("out", "elk_guilds", "swan_counts", "swan_counts_clean", "swan_counts_df", "Swan_guilds", "Swanspp"))
remove(list = c("filename", "i", "skips", "year"))
remove(swanton_data)
```
...
```{r}
remove(list = c("out", "elk_guilds", "ucsc_counts", "ucsc_counts_clean", "ucsc_counts_df", "UCSC_guilds", "UCSCspp"))
remove(list = c("filename", "i", "skips", "year"))
remove(ucsc_data)
```
...
```{r}
remove(list = c("spp_renamings", "spp_renamings_vec"))
remove(list = c("cleanElkhornData", "cleanElkhornPlot"))
```
...
```{r}
spp_clean_tbl <- read_csv("data/community/species-table-clean-2022-05-10.csv", col_types = "c")
