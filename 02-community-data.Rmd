...
```{r}
spp_renamings <- read_csv("data/community/species-table-luong-qin-2022-04-13.csv")
...
```{r}
remove(list = c("out", "swan_counts", "swan_counts_clean", "swan_counts_df", "Swan_guilds", "Swanspp"))
remove(list = c("filename", "i", "skips", "year"))
remove(swanton_data)
```
...
```{r}
remove(list = c("out", "ucsc_counts", "ucsc_counts_clean", "ucsc_counts_df", "UCSC_guilds", "UCSCspp"))
remove(list = c("filename", "i", "skips", "year"))
remove(ucsc_data)
```
...
Automatically resolve duplicated/misspelling species names, using the taxize package. 

```{r, eval=FALSE}
taxize::get_gbifid_(all_spp) %>%
...
```{r}
match_spp_tbl <- read_rds("data/community/species-table-taxize-2022-04-30.rds") %>%
