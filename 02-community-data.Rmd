...
write_csv(jrgce_data, paste0(.path$com_tidy, "jrgce.csv"))
rm(list = ls())
```
...
write_csv(mclexp_data, paste0(.path$com_tidy, "mclexp.csv"))
rm(list = ls())
```
...
write_csv(scide_data, paste0(.path$com_tidy, "scide.csv"))
rm(list = ls())
```
...
write_csv(angelo_data, paste0(.path$com_tidy, "angelo.csv"))
rm(list = ls())
```
...
write_csv(carrizo_data, paste0(.path$com_tidy, "carrizo.csv"))
rm(list = ls())
```
...
eb_com_tbl %>%
  ...
  write_csv(paste0(.path$com_tidy, "pleasantonridge.csv"))
```
...
eb_com_tbl %>%
  ...
  mutate(site = "sunol") %>%
  write_csv(paste0(.path$com_tidy, "sunol.csv"))
```
...
eb_com_tbl %>%
  ...
  write_csv(paste0(.path$com_tidy, "vascocaves.csv"))
rm(list = ls())
```
...
write_csv(jasper_data, paste0(.path$com_tidy, "jasper.csv"))
rm(list = ls())
```
