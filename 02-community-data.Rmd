...
carrizo_com <- readxl::read_xlsx(
  ...
  select(
    ...
    intercept = Count # or Count2 = Updated number of hits, where any species that was present but not hit were given a 1
  ) %>%
  filter(intercept > 0, !is.na(intercept))

carrizo_spp <- read_csv(
  "data/community/raw/Carrizo/2022-06-28/Carrizo_global_species list.csv",
  col_types = "c"
) %>%
...
carrizo_data <- carrizo_com %>%
  ...
  select(site, year, plot, species_code, species_name, guild, abund, abund_type) %>%
  arrange(site, year, plot, species_name)
