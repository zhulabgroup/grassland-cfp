...
These grassland cover data were collected as part of the Grassland Monitoring Project by the UC Berkeley Range Lab, Professor James Bartolome, PI (jwbart@berkeley.edu),
from 2003-2011, in the East Bay Regional Park District's Morgan Territory Regional Preserve.
...
mt_com_tbl <- readxl::read_xlsx(
  ...
  left_join(mt_spp_tbl, by = "species_code") %>%
  mutate(abund_type = "point_intercept") %>%
  filter(
    !(species %in% c("litter", "Moss", "no data collected", "Rock", "soil")),
    !str_detect(species, "unknown|Unknown"),
    is.na(guild) | guild != "UUU"
  ) %>%
  mutate(site = "morganterritory", year = as.integer(year)) %>%
  select(site, year, plot, species, guild, abund, abund_type) %>%
  right_join(
    mt_plt_tbl %>%
...
mt_com_tbl %>%
  write_csv("tmp/mt-community.csv")

mt_com_tbl %>%
  ungroup() %>%
  distinct(species, guild) %>%
  arrange(species) %>%
  write_csv("tmp/mt-species.csv")
```
