...
jrgce_data <- pin_tbl %>%
  ...
  select(site, year, plot, trt, species, guild, abund, abund_type) %>%
...
carrizo_data <- carrizo_data %>%
  ...
  mutate(
    ...
    guild = as.character(guild), abund_type = "intercept"
  ) %>%
  arrange(site, year, plot, species)
...
elkhorn_data <- elkhorn_data %>%
  ...
  mutate(year = as.integer(year), species = as.character(species) %>% str_trim(), guild = as.character(guild), abund = as.integer(abund), abund_type = "intercept") %>%
  arrange(site, year, plot, species)
...
swanton_data <- swanton_data %>%
  as_tibble() %>%
  select(site, year, plot = rep, species = species.name, guild, abund = intercepts) %>%
  mutate(year = as.integer(year), species = as.character(species) %>% str_trim(), guild = as.character(guild), abund_type = "intercept") %>%
  arrange(site, year, plot, species)
...
# read from CSV again to ensure correct typing
(read_csv("data/community/tidy/angelo.csv", col_types = "cicccdc") %>%
  ...
  write_rds("data/community/all-obs.rds")
```
