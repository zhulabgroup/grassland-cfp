...
Species renaming data.

```{r}
spp_renamings <- read_csv(
  "data/community/species-table-expert-2022-04-13.csv",
  col_types = "ccc"
) %>%
...
```{r}
rm(carrizo, carrizo_extras, carrizo_forms, carrizo_spp)
rm(carrizo_data)
```
...
```{r}
spp_tbl <- readxl::read_xlsx("data/community/raw/Dudney/_VegSpCodeAttributes_2010.xlsx") %>%
  mutate(
...
com_tbl <- read_csv("data/community/raw/Dudney/EBRPD_2002thru2012_Dec2013_BRXX AVXX updated.csv",
  col_types = "cicidc"
) %>%
  rename(plot = plot.ID, species_code = species) %>%
  group_by(site, year, plot, species_code) %>%
  summarize(abund = sum(point)) %>%
  left_join(spp_tbl, by = "species_code") %>%
  mutate(abund_type = "point_intercept") %>%
  select(site, year, plot, species_code, species, guild, abund, abund_type)
```

Validation with Joan's functional group (guild) spreadsheet.

```{r, eval=FALSE}
guild_sum_tbl1 <- readxl::read_xlsx("data/community/raw/Dudney/FunctionalGroups_EBParks.xlsx",
  range = readxl::cell_cols("A:E")
) %>%
  rename(plot = plot.ID, guild = fungrp)

guild_sum_tbl2 <- read_csv("data/community/raw/Dudney/EBRPD_2002thru2012_Dec2013_BRXX AVXX updated.csv",
  col_types = "cicidc"
) %>%
  rename(plot = plot.ID, species_code = species) %>%
  left_join(spp_tbl, by = "species_code") %>%
...
inner_join(guild_sum_tbl1, guild_sum_tbl2, by = c("site", "year", "plot", "guild")) %>%
...
Josie's email on 5/25/22: "Unfortunately, the point intercept data was only collected 2000-2006 AND there was a sharp decline in sampling effort for the 2006 year - so only a handful of transects have multiple years of data. I think we can drop these data from the list of potential additions for that reason."
...
```{r}
rm(spp_renamings, spp_renamings_vec)
...
First, read directly from CSV file(s) for all experimental and observational data. Make sure data types are correct. 
...
Prepare a master species list from tidy data.

```{r}
spp_tidy_tbl <- tibble(
  species =
    c(all_exp_data$species, all_obs_data$species) %>%
      unique() %>%
      sort()
)
```
...
```{r, eval=FALSE}
taxize::get_gbifid_(spp_tidy_tbl$species) %>%
  as_tibble_col(column_name = "gbif") %>%
  mutate(queryname = spp_tidy_tbl$species) %>%
  unnest(cols = c(gbif)) %>%
  select(queryname, everything()) %>%
  write_rds("data/community/species-table-taxize-2022-05-02.rds")
```

Automatic match original names with taxize names based on the conversation with Justin Luong, 2/15/22.
...
```{r, eval=FALSE}
spp_taxize_tbl <- read_rds("data/community/species-table-taxize-2022-05-02.rds") %>%
...
Manual correction based on the look-up table complied by Justin Luong and Clara Qin.

```{r, eval=FALSE}
spp_expert_tbl <- read_csv("data/community/species-table-expert-2022-04-13.csv", col_types = "ccc") %>%
  rename_with(~ tolower(gsub(" ", "_", .x, fixed = TRUE)))
```

Combining all species name corrections: taxize, expert, and drop genus-only "species."

```{r, eval=FALSE}
spp_clean_tbl <- spp_tidy_tbl %>%
  select(tidy_name = species) %>%
  left_join(
    spp_taxize_tbl %>%
      select(tidy_name = queryname, taxize_name = canonicalname),
    by = "tidy_name"
  ) %>%
  left_join(
    spp_expert_tbl %>%
      select(taxize_name = original_name, expert_name = revised_name, expert_issue = issue),
    by = "taxize_name"
  ) %>%
  mutate(
    clean_name = if_else(
      (!is.na(expert_name) | !is.na(expert_issue)), # if expert correction or drop
      expert_name,
      taxize_name
    )
  ) %>%
  mutate(
    clean_name = if_else(
      str_detect(clean_name, " "), # genus-species combo has a space; otherwise drop genus-only "species"
      clean_name,
      "DROP"
    ),
    clean_name = na_if(clean_name, "DROP")
  )

write_csv(spp_clean_tbl, "data/community/species-table-clean-2022-05-02.csv")
```

Finally, replace clean species names in all data and save to RDS. The species clean table is based on the above code, supplemented by Justin's manual adjustments on 5/6/22 and Kai's edits on 5/10/22.

Updated on 6/8/22. Directly use the species table reviewed by Justin.

```{r}
spp_clean_tbl <- read_csv("data/community/species-table-clean-2022-06-08.csv", col_types = "c")

all_exp_data %>%
  rename(tidy_name = species) %>%
  inner_join(spp_clean_tbl %>% select(tidy_name, clean_name), by = "tidy_name") %>%
  drop_na(clean_name) %>%
  filter(abund > 0) %>%
  select(
    site, year, plot,
...
```{r}
rm(all_exp_data, all_obs_data, spp_clean_tbl, spp_tidy_tbl)
# rm(spp_expert_tbl, spp_taxize_tbl)
```
