...
```{r eval=FALSE}
gbif_spp <- taxize::get_gbifid_(all_spp) %>% 
  as_tibble_col(column_name = "gbif") %>% 
  mutate(queryname = all_spp) %>% 
  unnest(cols = c(gbif)) %>% 
  select(queryname, everything())
write_rds(gbif_spp, "data/occurrence/gbif/gbif-spp.rds")
```

Then, choose the top match.

```{r}
gbif_spp <- read_rds("data/occurrence/gbif/gbif-spp.rds") %>% 
  group_by(queryname) %>% 
