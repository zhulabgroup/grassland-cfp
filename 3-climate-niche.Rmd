...
## Compare bioclimatic variables

Download (almost) all the bioclimatic variables, defined by [WorldClim](https://www.worldclim.org/data/bioclim.html) and [CHELSA manual](https://chelsa-climate.org/wp-admin/download-page/CHELSA_tech_specification_V2.pdf). Extract at GBIF locations. 

```{r extract-bioclim, eval=FALSE}
source("scripts/extract-bioclim-gbif.R")
rm(list = ls())
```

Compare bioclimatic variables by correlation analysis. We see high correlation within different temperature variables (bio1, 5, 6, 8-11), precipitation variables (bio12-14, 16-19), and vpd(vpd_max, mean, min) variables.
...
```{r estimate-thin-niche, eval=FALSE}
source("scripts/estimate-thin-occ-niche.R")
```

Post-thinning analysis. The comparison shows full vs. thinned niche estimates are very similar.

```{r compare-thin-niche}
source("scripts/compare-thin-spp-niche.R")
```
