...
#  filter(guild == "Ann" | guild == "Nat" | guild == "Forbs")
  filter(guild == "Nat")
...
ggplot(gcov_all_means, aes(x = year, y = meanrelcov, color = guild)) +
  ...
  expand_limits(y = 0)+
  facet_grid(guild~site, scales = "free_x")


## Overall realtive native cover for table
overall_mean_natrelcov <- gcov_all %>%
  group_by(site, guild) %>%
  summarise(serelcov = (std.error(meancov)),
            meanrelcov = mean(meancov)) %>%
  filter(guild == "Nat")
...
gcov_all_nat <- gcov_all %>%
  filter(guild == "Nat") %>%
  mutate(year2 = year,
         # we rescale the year to improve the model and prevent eigenvalue warnings
         year = abs(year2-1983))

gcov_all_nat$year <- as.numeric(gcov_all_nat$year)

# Angelo
summary(glm(meancov ~ year ,  
              data = gcov_all_nat[gcov_all_nat$site == "Angelo",]))

# Carrizo - has a problem, inconsistent plots -- ignore data?
summary(glm(meancov ~ year, 
              data = gcov_all_nat[gcov_all_nat$site == "Carrizo",]))

# Elkhorn
summary(glm(meancov ~ year,
              data = gcov_all_nat[gcov_all_nat$site == "Elkhorn",]))

# Jasper
summary(glm(meancov ~ year, 
              data = gcov_all_nat[gcov_all_nat$site == "Jasper",]))

# McL Ann
summary(glm(meancov ~ year, 
              data = gcov_all_nat[gcov_all_nat$site == "McL Ann",]))

# McL Serp
summary(glm(meancov ~ year, 
              data = gcov_all_nat[gcov_all_nat$site == "McL Serp",]))

# Swanton
summary(glm(meancov ~ year, 
              data = gcov_all_nat[gcov_all_nat$site == "Swanton",]))

# UCSC
summary(glm(meancov ~ year,
            data = gcov_all_nat[gcov_all_nat$site == "UCSC",]))
...
ggplot(all_turn_means, aes(x = precip, y = meanturn)) +
  ...
  ggplot(all_turn_means, aes(x = year, y = meanturn)) +
    ...
    facet_wrap(~site, ncol = 8, scales = "free_x") +
    theme(legend.position = "none")
...
Here, we'll try to analyze unweighted CTI for all of the sites at once - this means that rare and common species will have the same weight towards the CTI of a site over time. 

```{r import uwCTI data}
uwCTI_all_raw <- read.csv("Analysis/CTI Data/AllSite_CTI_08_2020.csv")
  
uwCTI_all_raw <- uwCTI_all_raw %>%
  filter(species.name != "Linanthus jepsonii",
         species.name != "Linanthus latisectus")

uwCTI_all <- uwCTI_all_raw %>%
  filter(relcov >= 0) %>%
  group_by(site, year, plot) %>%
  summarise(plotCTI = mean(meanT),
            plotCPI = mean(meanp))

uwCTI_plot_means <- uwCTI_all %>%
...
grid.arrange(
ggplot(uwCTI_site_means, aes(x = year, y = meanCTI)) +
  geom_line() +
  geom_point() +
  geom_errorbar(aes(ymin = (meanCTI - seCTI), ymax = (meanCTI + seCTI))) +
  labs(title = "All sites - unweighted Community Temperature Index",
       x = "Year",
       y = "uwCTI") +
  theme_bw() +
  theme(legend.position = "none") +
  facet_wrap(~site, scales = "free", ncol = 8) # + geom_smooth(method = "lm")
,
ggplot(uwCTI_site_means, aes(x = year, y = meanCPI)) +
  geom_line() +
  geom_point() +
...
```{r uwCTI MODELS}
uwCTI_models <- uwCTI_plot_means %>%
  mutate(year2 = year,
         # we rescale the year to improve the model and prevent eigenvalue warnings
         year = abs(year2-1983))
uwuwCTI_models$year <- as.numeric(uwuwCTI_models$year)

# Angelo
summary(glm(meanCTI ~ year,
              data = uwCTI_models[uwCTI_models$site == "Angelo",]))

# Carrizo - has a problem, inconsistent plots -- ignore data for now
summary(glm(meanCTI ~ year,
              data = uwCTI_models[uwCTI_models$site == "Carrizo",]))

# Elkhorn
summary(glm(meanCTI ~ year,
              data = uwCTI_models[uwCTI_models$site == "Elkhorn",]))

# Jasper
summary(glm(meanCTI ~ year,
              data = uwCTI_models[uwCTI_models$site == "Jasper",]))

# McL Ann
summary(glm(meanCTI ~ year,
              data = uwCTI_models[uwCTI_models$site == "McL Ann",]))

# McL Serp
summary(glm(meanCTI ~ year,
              data = uwCTI_models[uwCTI_models$site == "McL Serp",]))

# Swanton
summary(glm(meanCTI ~ year,
              data = uwCTI_models[uwCTI_models$site == "Swanton",]))

# UCSC
summary(glm(meanCTI ~ year,
              data = uwCTI_models[uwCTI_models$site == "UCSC",]))
```

```{r uwCPI MODELS}
uwCPI_models <- uwCTI_plot_means %>%
  mutate(year2 = year,
         # we rescale the year to improve the model and prevent eigenvalue warnings
         year = abs(year2-1983))
uwCPI_models$year <- as.numeric(uwCPI_models$year)

# Angelo
summary(glm(meanCPI ~ year,
              data = uwCPI_models[uwCPI_models$site == "Angelo",]))

# Carrizo - has a problem, inconsistent plots -- ignore data for now
summary(glm(meanCPI ~ year,
              data = uwCPI_models[uwCPI_models$site == "Carrizo",]))

# Elkhorn
summary(glm(meanCPI ~ year,
              data = uwCPI_models[uwCPI_models$site == "Elkhorn",]))

# Jasper
summary(glm(meanCPI ~ year,
              data = uwCPI_models[uwCPI_models$site == "Jasper",]))

# McL Ann
summary(glm(meanCPI ~ year,
              data = uwCPI_models[uwCPI_models$site == "McL Ann",]))

# McL Serp
summary(glm(meanCPI ~ year,
              data = uwCPI_models[uwCPI_models$site == "McL Serp",]))

# Swanton
summary(glm(meanCPI ~ year,
              data = uwCPI_models[uwCPI_models$site == "Swanton",]))

# UCSC
summary(glm(meanCPI ~ year,
              data = uwCPI_models[uwCPI_models$site == "UCSC",]))
```

# Changes in wCTI over time

Here, we'll try to analyze CTI for all of the sites at once. 

```{r import CTI data}
CTI_all_raw <- read.csv("Analysis/CTI Data/AllSite_CTI_08_2020.csv")
  
CTI_all_raw <- CTI_all_raw %>%
  filter(species.name != "Linanthus jepsonii",
         species.name != "Linanthus latisectus")
...
grid.arrange(
ggplot(CTI_site_means, aes(x = year, y = meanCTI)) +
  geom_line() +
  geom_point() +
  geom_errorbar(aes(ymin = (meanCTI - seCTI), ymax = (meanCTI + seCTI))) +
...
```{r compare guild average CPI/CTI}
CTI_byspp <- CTI_all_raw %>%
  group_by(site, guild) %>%
  summarise(meanSTI = mean(meanT),
            meanSPI = mean(meanp),
            seSTI = std.error(meanT),
            seSPI = std.error(meanp))

ggplot(CTI_byspp, aes(x = guild, y = meanSTI)) +
  geom_line() +
  geom_point() +
  geom_errorbar(aes(ymin = (meanSTI - seSTI), ymax = (meanSTI + seSTI))) +
  labs(title = "All sites - Community Temperature Index",
       x = "Year",
       y = "CTI") +
  theme_bw() +
...
# Angelo
summary(glm(meanCTI ~ year,
              data = uwCTI_models[CTI_models$site == "Angelo",]))
...
# McL Serp
summary(glm(meanCTI ~ year,
              data = CTI_models[CTI_models$site == "McL Serp",]))t_means %>%
  mutate(year2 = year,
         # we rescale the year to improve the model and prevent eigenvalue warnings
         year = abs(year2-1983))
...
## CTI vs actual temp
```{r adding weather}
wtryr_weather <- read.csv("Analysis/ClimData/PRISM_wtryr_alldata.csv") %>%
   rename("year" = "wateryear")
```

```{r join CTI/temp}
CTI_Clim_Rich_byplot <- left_join(CTI_plot_means, wtryr_weather)
CTI_Clim_Rich_bysite <- left_join(CTI_site_means, wtryr_weather)

## Mean T and CTI
ggplot(CTI_Clim_Rich_bysite, aes(x = mean_meanT, y = meanCTI, color = site)) +
  geom_point() +
  geom_errorbar(aes(ymin = meanCTI-seCTI, ymax = meanCTI+seCTI)) +
  theme_bw() +
  theme(legend.position = "none") +
...
### --- this one is huge!! --- ###
## Grow_VPD_min and CTI
ggplot(CTI_Clim_Rich_bysite, aes(x = grow_vpdmin, y = meanCPI, color = site)) +
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = meanCPI-seCPI, ymax = meanCPI+seCPI)) +
  theme_bw() +
  theme(legend.position = "none") +
...
summary(glm(meanCTI ~ grow_vpdmin,
              data = CTI_Clim_Rich_byplot[CTI_Clim_Rich_byplot$site == "Angelo",]))
summary(glm(meanCTI ~ grow_vpdmin,
              data = CTI_Clim_Rich_byplot[CTI_Clim_Rich_byplot$site == "Carrizo",]))
summary(glm(meanCTI ~ grow_vpdmin,
              data = CTI_Clim_Rich_byplot[CTI_Clim_Rich_byplot$site == "Elkhorn",]))
summary(glm(meanCTI ~ grow_vpdmin,
              data = CTI_Clim_Rich_byplot[CTI_Clim_Rich_byplot$site == "Jasper",]))
summary(glm(meanCTI ~ grow_vpdmin,
              data = CTI_Clim_Rich_byplot[CTI_Clim_Rich_byplot$site == "McL Ann",]))
summary(glm(meanCTI ~ grow_vpdmin,
              data = CTI_Clim_Rich_byplot[CTI_Clim_Rich_byplot$site == "McL Serp",]))
summary(glm(meanCTI ~ grow_vpdmin,
              data = CTI_Clim_Rich_byplot[CTI_Clim_Rich_byplot$site == "Swanton",]))
summary(glm(meanCTI ~ grow_vpdmin,
              data = CTI_Clim_Rich_byplot[CTI_Clim_Rich_byplot$site == "UCSC",]))


## Mean_VPD_min and CTI
ggplot(CTI_Clim_Rich_bysite, aes(x = mean_vpdmin, y = meanCTI, color = site)) +
  geom_point() +
  geom_errorbar(aes(ymin = meanCTI-seCTI, ymax = meanCTI+seCTI)) +
  theme_bw() +
  theme(legend.position = "none") +
...
summary(glm(meanCTI ~ mean_vpdmin,
              data = CTI_Clim_Rich_byplot[CTI_Clim_Rich_byplot$site == "Angelo",]))
summary(glm(meanCTI ~ mean_vpdmin,
              data = CTI_Clim_Rich_byplot[CTI_Clim_Rich_byplot$site == "Carrizo",]))
summary(glm(meanCTI ~ mean_vpdmin,
...
## Min T and CTI
ggplot(CTI_Clim_Rich, aes(x = mean_minT, y = meanCTI, color = site)) +
  geom_point() +
  geom_errorbar(aes(ymin = meanCTI-seCTI, ymax = meanCTI+seCTI)) +
  theme_bw() +
  theme(legend.position = "none") +
...
## Max T and CTI
ggplot(CTI_Clim_Rich, aes(x = mean_maxT, y = meanCTI, color = site)) +
  geom_point() +
  geom_errorbar(aes(ymin = meanCTI-seCTI, ymax = meanCTI+seCTI)) +
  theme_bw() +
  theme(legend.position = "none") +
...
## Win min VPD and CTI
ggplot(CTI_Clim_Rich, aes(x = win_vpdmin, y = meanCTI, color = site)) +
  geom_point() +
  theme_bw() +
  theme(legend.position = "none") +
  facet_wrap(~site, scales = "free", ncol = 4) + 
...
## Win max VPD and CTI
ggplot(CTI_Clim_Rich, aes(x = win_vpdmax, y = meanCTI, color = site)) +
  geom_point() +
  theme_bw() +
  theme(legend.position = "none") +
  facet_wrap(~site, scales = "free", ncol = 4) + 
...
## Min VPD and CTI
ggplot(CTI_Clim_Rich, aes(x = mean_vpdmin, y = meanCTI, color = site)) +
  geom_point() +
  theme_bw() +
  theme(legend.position = "none") +
  facet_wrap(~site, scales = "free", ncol = 4) + 
...
## Min VPD and CTI
ggplot(CTI_Clim_Rich, aes(x = mean_vpdmin, y = meanCPI, color = site)) +
  geom_point() +
  theme_bw() +
  theme(legend.position = "none") +
  facet_wrap(~site, scales = "free", ncol = 4) + 
...
summary(glm(meanCPI ~ precip,
              data = CTI_Clim_Rich[CTI_Clim_Rich$site == "Angelo",]))
summary(glm(meanCPI ~ precip,
              data = CTI_Clim_Rich[CTI_Clim_Rich$site == "Carrizo",]))
summary(glm(meanCPI ~ precip,
              data = CTI_Clim_Rich[CTI_Clim_Rich$site == "Elkhorn",]))
summary(glm(meanCPI ~ precip,
              data = CTI_Clim_Rich[CTI_Clim_Rich$site == "Jasper",]))
summary(glm(meanCPI ~ precip,
              data = CTI_Clim_Rich[CTI_Clim_Rich$site == "McL Ann",]))
summary(glm(meanCPI ~ precip,
              data = CTI_Clim_Rich[CTI_Clim_Rich$site == "McL Serp",]))
summary(glm(meanCPI ~ precip,
              data = CTI_Clim_Rich[CTI_Clim_Rich$site == "Swanton",]))
summary(glm(meanCPI ~ precip,
              data = CTI_Clim_Rich[CTI_Clim_Rich$site == "UCSC",]))

## Minimum T and CTI
ggplot(CTI_Clim_Rich, aes(x = tmin, y = meanCTI, color = site)) +
  geom_point() +
  theme_bw() +
  theme(legend.position = "none") +
  facet_wrap(~site, scales = "free", ncol = 4) + 
...
ggplot(CTI_Clim_Rich, aes(x = meanCTI, y = meanCPI, color = site)) +
  geom_point() +
  theme_bw() +
  theme(legend.position = "none") +
  facet_wrap(~site, scales = "free", ncol = 4) + 
...
# Comparing CTI of Native and Non-Native species
```{r CTI by nat/non-nat}
STI_byspecies_raw <- CTI_all_raw %>%
  group_by(species.name, guild, site) %>%
  summarise(SpTI = mean(meanT),
            SpPI = mean(meanp)) %>%
  mutate(guild = recode_factor(guild, 
                               ...
                               "EAF" = "Non-native Annual Forb",
                               "EAG" = "Non-native Annual Grass",
                               "EPF" = "Non-native Perennial Forb",
                               "EPG" = "Non-native Perennial Grass",
                               "NPS" = "Native Perennial Shrub",
                               "NPT" = "Native Perennial Tree",
                               "NAGr" = "Native Annual Graminiod",
                               "NPGr" = "Native Perennial Graminiod"
                               )) %>%
  filter(guild != "NANANA") %>%
  separate(guild, into = c("Provenance", "AnnPer", "Lifeform"), sep = " ")

STI_byspecies <- STI_byspecies_raw %>%
  group_by(site, Provenance) %>%
  summarise(STI = mean(SpTI),
            SPI = mean(SpPI),
            seSTI = std.error(SpTI),
            seSPI = std.error(SpPI))


STI_graph <- ggplot(STI_byguild, aes(x = Provenance, y = STI, color = Provenance)) +
  geom_point() +
  geom_errorbar(aes(ymin = STI - seSTI, ymax = STI + seSTI)) +
  labs(y = "Mean Species \n Temperature Index") +
  theme_bw() +
...
SPI_graph <- ggplot(STI_byguild, aes(x = Provenance, y = SPI, color = Provenance)) +
  geom_point() +
  geom_errorbar(aes(ymin = SPI - seSPI, ymax = SPI + seSPI)) +
  labs(y = "Mean Species \n Precipitation Index") +
  theme_bw() +
  theme(legend.position = "none",
...
nat_exo <- g_legend(STI_graph)

grid.arrange(arrangeGrob(STI_graph + theme(legend.position = "none"), SPI_graph, ncol=1, nrow=2),
         arrangeGrob(nat_exo, ncol=1, nrow=1), heights=c(6,0), widths=c(4,1))


# t-tests
## STI

t.test(SpTI ~ Provenance , data = STI_byspecies_raw)

# Angelo
t.test(SpTI ~ Provenance, data = STI_byspecies_raw[STI_byspecies_raw$site == "Angelo",])

# Carrizo
t.test(SpTI ~ Provenance, data = STI_byspecies_raw[STI_byspecies_raw$site == "Carrizo",])

# Elkhorn
t.test(SpTI ~ Provenance, data = STI_byspecies_raw[STI_byspecies_raw$site == "Elkhorn",])

# Jasper
t.test(SpTI ~ Provenance, data = STI_byspecies_raw[STI_byspecies_raw$site == "Jasper",])

# McL Ann
t.test(SpTI ~ Provenance, data = STI_byspecies_raw[STI_byspecies_raw$site == "McL Ann",])

# McL Serp
t.test(SpTI ~ Provenance, data = STI_byspecies_raw[STI_byspecies_raw$site == "McL Serp",])

# Swanton
t.test(SpTI ~ Provenance, data = STI_byspecies_raw[STI_byspecies_raw$site == "Swanton",])

# UCSC
t.test(SpTI ~ Provenance, data = STI_byspecies_raw[STI_byspecies_raw$site == "UCSC",])


## SPI
# Angelo
t.test(SpPI ~ Provenance, data = STI_byspecies_raw[STI_byspecies_raw$site == "Angelo",])

# Carrizo
t.test(SpPI ~ Provenance, data = STI_byspecies_raw[STI_byspecies_raw$site == "Carrizo",])

# Elkhorn
t.test(SpPI ~ Provenance, data = STI_byspecies_raw[STI_byspecies_raw$site == "Elkhorn",])

# Jasper
t.test(SpPI ~ Provenance, data = STI_byspecies_raw[STI_byspecies_raw$site == "Jasper",])

# McL Ann
t.test(SpPI ~ Provenance, data = STI_byspecies_raw[STI_byspecies_raw$site == "McL Ann",])

# McL Serp
t.test(SpPI ~ Provenance, data = STI_byspecies_raw[STI_byspecies_raw$site == "McL Serp",])

# Swanton
t.test(SpPI ~ Provenance, data = STI_byspecies_raw[STI_byspecies_raw$site == "Swanton",])

# UCSC
t.test(SpPI ~ Provenance, data = STI_byspecies_raw[STI_byspecies_raw$site == "UCSC",])
...
ggplot(grich_all_means, aes(x = year, y = meanrich, color = guild)) +
  ...
  scale_y_continuous(limits = c(0,NA)) + 
  expand_limits(y = 0) +
  facet_wrap(~site, scales = "free_x", ncol = 8) 
# + geom_smooth(method='lm')
,
 ggplot(gcov_all_means, aes(x = year, y = meanrelcov, color = guild)) +
  geom_line() +
...
ggplot(all_turn_means, aes(x = year, y = meanturn)) +
    ...
    labs(title = "Community turnover over time",
         x = "Year",
         y = "Turnover (# lost + gained/total # spp.)") +
    facet_wrap(~site, ncol = 8, scales = "free_x") +
    theme_bw() +
...
ggplot(CTI_site_means, aes(x = year, y = meanCTI)) +
  ...
  facet_wrap(~site, scales = "free", ncol = 8) # + geom_smooth(method = "lm")
,
ggplot(CTI_site_means, aes(x = year, y = meanCPI)) +
  geom_line() +
  geom_point() +
  geom_errorbar(aes(ymin = (meanCPI - seCPI), ymax = (meanCPI + seCPI))) +
  labs(title = "All sites - Community Precipitation Index",
