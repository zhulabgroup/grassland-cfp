...
# Comparisons of rates of change over time
```{r}
...
# Top five species over time

```{r}
top3_ang <- rel_all %>%
  filter(!is.na(relcov),
         site == "Angelo") %>%
  complete(year, site, plot, species.name) %>%
  mutate(relcov = replace_na(relcov, 0))

rel_all_ang <- rel_all %>% filter(site == "Angelo")

ang_ranks <- aggregate(
  relcov ~ species.name * year * site * plot, 
                    data = top3_ang, 
                    FUN = mean)

ggplot(ang_ranks, aes(year, relcov, color = species.name)) + 
  geom_line(size = 2) + coord_polar() + theme_bw() +
  ggtitle("Dominant species abundances \n Annually burned vs unburned plots, Konza \n")


ggplot(top3_ang, aes(x = year, y = relcov_avg, color = species.name)) +
  #geom_errorbar(aes(ymin = mean_relcov - se_relcov, ymax = mean_relcov + se_relcov)) +
  geom_line() +
  theme_bw() +
