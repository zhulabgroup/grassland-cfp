...
```{r com data import, echo=FALSE, message=FALSE}

angelo_data <- read.csv("Analysis/ComData/angelo_data.csv") %>%
  dplyr::select(-X)  %>%
  mutate(site = "Angelo") 
ang_data_rel <- angelo_data %>%
  dplyr::select(year, plot, species.name, cover) %>%
  group_by(plot, year) %>%
...
car_data_rel <- car_data %>%
  dplyr::select(year, plot, species.name, Intercepts) %>%
...
ang_grich <- angelo_data %>%
  ...
  group_by(year, plot) %>%
  dplyr::select(plot, year, rich, guild)  %>%
  spread(guild, rich) %>%
  replace(is.na(.), 0) %>%
  ...
  mutate(site = "Angelo") %>%
  dplyr::select(site, year, plot, guild, rich)
...
all.grich.means <- all.grich %>%
  group_by(site, guild, year, site2) %>%
...
## Graphs for pure richness

```{r guild rich graphs}
# all guilds, one graph
...
(ggplot(guild.grich) +
    ...
    labs(title = "Richness of native and exotic species in 8 CA grasslands",
       x = "Year",
       y = "Richness") +
    facet_wrap(~site2, nrow = 1, scales = "free"))


## Overall richness
g.all.grich <- all.grich.means %>% filter(guild == "all")
## adding significance info to graphs
all.grich.sig <- data.frame(site = c("Angelo", "Carrizo", "Elkhorn", "Jasper", "McL Ann", "McL Serp", "Swanton", "UCSC"),
                        sig = c("Yes", "No", "No", "No", "Yes", "Yes", "No", "Yes"))
g.all.grich <- left_join(g.all.grich, all.grich.sig)

(graph.all.rich <- ggplot(g.all.grich, aes(x = year, y = meanrich, color = site2)) +
    geom_point() +
...
## adding significance info to graphs
nat.grich.sig <- data.frame(site = c("Angelo", "Carrizo", "Elkhorn", "Jasper", "McL Ann", "McL Serp", "Swanton", "UCSC"),
                        sig = c("Yes", "No", "Yes", "No", "Yes", "Yes", "No", "Yes"))
g.nat.grich <- left_join(g.nat.grich, nat.grich.sig)

(graph.nat.rich <- ggplot(g.nat.grich, aes(x = year, y = meanrich, color = site2)) +
    geom_point() +
    geom_line(aes(linetype = sig)) +
  geom_errorbar(aes(ymin = meanrich-serich, ymax = meanrich+serich)) +
...
exo.grich.sig <- data.frame(site = c("Angelo", "Carrizo", "Elkhorn", "Jasper", "McL Ann", "McL Serp", "Swanton", "UCSC"),
                        sig = c("No", "No", "Yes", "Yes", "Yes", "No", "No", "Yes"))
g.exo.grich <- left_join(g.exo.grich, exo.grich.sig)
...
ang_grich <- angelo_data %>%
  ...
  dplyr::select(plot, year, relrich, guild)  %>%
...
elk_grich <- elkhorn_data %>%
  ...
  mutate(totalrich = sum(rich),
         relrich = rich/totalrich)  %>%
  dplyr::select(rep, year, relrich, guild)  %>%
  spread(guild, relrich) %>%
  replace(is.na(.), 0) %>%
  add_column(NAF = 0) %>% add_column(NAG = 0) %>% add_column(NPS = 0) %>% add_column(NPT = 0) %>%
  mutate(Nat = (NAF+NAG+NAGr+NPF+NPG+NPGr+NPS+NPT),
...
jasp_grich <- jasper_data %>%
  ...
  mutate(totalrich = sum(rich),
         relrich = rich/totalrich)  %>%
  dplyr::select(uniqueID, year, relrich, guild)  %>%
  spread(guild, relrich) %>%
  replace(is.na(.), 0) %>% 
  add_column(NAGr = 0) %>%  add_column(EAF = 0) %>% add_column(EPF = 0) %>%
  add_column(EPG = 0) %>%  add_column(NPGr = 0) %>% add_column(NPS = 0) %>%
...
ang_gcover <- angelo_data %>%
  ...
  group_by(year, plot) %>%
  mutate(sumcov = sum(cover),
         relcov = cover/sumcov) %>% dplyr::select(plot, year, relcov, guild)  %>%
  spread(guild, relcov) %>%
  replace(is.na(.), 0) %>%
  add_column(NAG = 0) %>% add_column(NAGr = 0) %>% add_column(EPG = 0) %>%
  add_column(NPGr = 0) %>% add_column(NPS = 0) %>% add_column(NPT = 0) %>%
...
## graphs of native cover and cti/cpi
nat.cti.graph <- ggplot(nat.gcov, aes(x = meanrelcov, y = mean.CTI, color = site)) +
  geom_point() +
  labs(title = "",
...
nat.cpi.graph <- ggplot(nat.gcov, aes(x = meanrelcov, y = mean.CPI, color = site)) +
  geom_point() +
  labs(title = "",
       x = "Relative native cover",
       y = "CPI") +
...
cov.ct.leg <- g_legend(nat.cpi.graph)

grid.arrange(nat.cti.graph, 
             nat.cpi.graph + theme(legend.position = "none"),
             cov.ct.leg,
             nrow = 1)
  


## relative guild cover over time
grid.arrange(
