...
Multiple occurrence data are available in the CFP. We downloaded and compared BIEN, CCH, GBIF, iNat occurrence datasets. The comparison shows that GBIF is the most comprehensive in terms the record number and species number.
...
```{r occ-data-fig, fig.cap="Occurrence data comparison from multiple sources in study area. Randomly showing 10% of data points to avoid overplotting."}
occ_comp_gg
rm(list = ls())
```
...
```{r clim-data-fig, fig.cap="Climate data comparison from multiple sources (CHELSA, PRISM, and TerraClimate) in CFP. Each point corresponds to a species occurrence from the GBIF data. Solid is the 1:1 line, and dashed is the regression line, where statistics are reported on the topleft."}
