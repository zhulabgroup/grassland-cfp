...
```{r clim-data-fig, fig.width=10, fig.height=10, fig.cap="Climate data comparison from multiple sources (CHELSA, PRISM, and TerraClimate) in CFP. Each point corresponds to a species occurrence from the GBIF data. Solid is the 1:1 line, and dashed is the regression line, where statistics (correlation coefficient R and p-value) are reported on the top left."}
...
We present individual species distributions in geographical space (occurrence) and climate space (niche). The left panel, titled with the species name, shows GBIF occurrence records in the CFP. The right panel, titled with the GBIF record number, shows the same occurrence data in reference to their mean annual temperature and precipitation from CHELSA data. The scatters are summarized by axis rugs for marginal distributions and a Gaussian data ellipse at 95% confidence interval and a centroid cross (median) for the joint distribution. The centroids are then assigned as species climate niche estimates.
...
```{r cool-sp-fig, fig.width=8, fig.height=8*.618, fig.cap="Example species *Danthonia californica*"}
niche_gg["Danthonia californica"]
```

```{r warm-sp-fig, fig.width=8, fig.height=8*.618, fig.cap="Example species *Stipa pulchra*"}
