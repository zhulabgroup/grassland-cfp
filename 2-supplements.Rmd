...
```{r exp-extra-lme-tab}
source("scripts/test-exp-lme.R")
knitr::kable(extra_lme, caption = "Comparison of Community Temperature Index (CTI, °C) and Community Precipitation Index (CPI, mm) in additional treatments and experiments, pooling data from all years, reporting average difference in the metric between treatment and ambient plots, *p*-value, and significance level from linear mixed-effect model with a random intercept for each year (ns: *p* > 0.05, $*$: *p* ? 0.05, $**$: *p* ? 0.01, $***$: *p* ? 0.001). ")
rm(list = ls())
```
