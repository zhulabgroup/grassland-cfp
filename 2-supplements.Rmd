...
obs_cti_tab %>%
  knitr::kable(caption = "Trends of Community Temperature Index (CTI) across observational sites, reporting the linear slope estimate, standard error,  *p*-value, and significance level from *t*-test (ns: *p* > 0.05, $*$: *p* ≤ 0.05, $**$: *p* ≤ 0.01, $***$: *p* ≤ 0.001).")
```

```{r obs-cpi-tab}
obs_cpi_tab %>%
  knitr::kable(caption = "Trends of Community Precipitation Index (CPI) across observational sites, reporting the linear slope estimate, standard error,  *p*-value, and significance level from *t*-test (ns: *p* > 0.05, $*$: *p* ≤ 0.05, $**$: *p* ≤ 0.01, $***$: *p* ≤ 0.001).")
```

```{r exp-cti-tab}
exp_cti_tab %>%
  knitr::kable(caption = "Comparison of Community Temperature Index (CTI) in the Jasper Ridge Global Change Experiment, reporting average CTI in ambient plots and warming plots, their difference, *p*-value, and significance level from non-parametric two-sample Wilcoxon test (ns: *p* > 0.05, $*$: *p* ≤ 0.05, $**$: *p* ≤ 0.01, $***$: *p* ≤ 0.001).")
```

```{r exp-cpi-tab}
exp_cpi_tab %>%
  knitr::kable(caption = "Comparison of Community Precipitation Index (CPI) in the Jasper Ridge Global Change Experiment, reporting average CPI in ambient plots and warming plots, their difference, *p*-value, and significance level from non-parametric two-sample Wilcoxon test (ns: *p* > 0.05, $*$: *p* ≤ 0.05, $**$: *p* ≤ 0.01, $***$: *p* ≤ 0.001).")
rm(list = ls())
```
