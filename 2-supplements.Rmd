...
```{r obs-tmp-tab}
obs_tmp_tbl %>%
  knitr::kable(caption = "Trends of mean annual temperature (°C) across observational sites, reporting the linear slope estimate, standard error,  *p*-value, and significance level from *t*-test (ns: *p* > 0.05, $*$: *p* ≤ 0.05, $**$: *p* ≤ 0.01, $***$: *p* ≤ 0.001).")
```

```{r obs-ppt-tab}
obs_ppt_tbl %>%
  knitr::kable(caption = "Trends of total annual precipitation (mm) across observational sites, reporting the linear slope estimate, standard error,  *p*-value, and significance level from *t*-test (ns: *p* > 0.05, $*$: *p* ≤ 0.05, $**$: *p* ≤ 0.01, $***$: *p* ≤ 0.001).")
rm(list = ls())
```
...
#| fig.cap = "Comparing the ecological niches of species of different guilds in terms of (A) origin, (B) life history, and (C) functional group. Red Gaussian ellipses show the 95% confidence interval of the ecological niche of each guild."
...
```{r all-sp-niche-tab, eval=FALSE}
# CSV file for species niche table
source("scripts/print-sp-niche.R")
rm(list = ls())
```
...
obs_cti_tab %>%
  knitr::kable(caption = "Trends of Community Temperature Index (CTI, °C) across observational sites, reporting the linear slope estimate, standard error,  *p*-value, and significance level from *t*-test (ns: *p* > 0.05, $*$: *p* ≤ 0.05, $**$: *p* ≤ 0.01, $***$: *p* ≤ 0.001).")
```

```{r obs-cpi-tab}
obs_cpi_tab %>%
  knitr::kable(caption = "Trends of Community Precipitation Index (CPI, mm) across observational sites, reporting the linear slope estimate, standard error,  *p*-value, and significance level from *t*-test (ns: *p* > 0.05, $*$: *p* ≤ 0.05, $**$: *p* ≤ 0.01, $***$: *p* ≤ 0.001).")
```
...
```{r exp-cti-tab}
exp_cti_tab %>%
  knitr::kable(caption = "Comparison of Community Temperature Index (CTI, °C) in the Jasper Ridge Global Change Experiment, reporting average CTI in ambient plots and warming plots, their difference, *p*-value, and significance level from non-parametric two-sample Wilcoxon test (ns: *p* > 0.05, $*$: *p* ≤ 0.05, $**$: *p* ≤ 0.01, $***$: *p* ≤ 0.001).")
```

```{r exp-cpi-tab}
exp_cpi_tab %>%
  knitr::kable(caption = "Comparison of Community Precipitation Index (CPI, mm) in the Jasper Ridge Global Change Experiment, reporting average CPI in ambient plots and warming plots, their difference, *p*-value, and significance level from non-parametric two-sample Wilcoxon test (ns: *p* > 0.05, $*$: *p* ≤ 0.05, $**$: *p* ≤ 0.01, $***$: *p* ≤ 0.001).")
rm(list = ls())
```

```{r exp-lme-tab}
source("scripts/test-exp-lme.R")
knitr::kable(jrgce_warm_lme, caption = "Comparison of Community Temperature Index (CTI, °C) and Community Precipitation Index (CPI, mm) in the Jasper Ridge Global Change Experiment (JRGCE), pooling data from all years in each phase, reporting average difference in the metric between warming and ambient plots, *p*-value, and significance level from linear mixed-effect model with a random intercept for each year (ns: *p* > 0.05, $*$: *p* ≤ 0.05, $**$: *p* ≤ 0.01, $***$: *p* ≤ 0.001). ")
rm(list = ls())
```

```{r guild-exp-fig, fig.width=10, fig.height=6.18/2*3}
...
#| fig.cap="In most years, drought treatment did not lead to significant community compositional shift measured by CTI (°C) and CPI (mm) in the Santa Cruz International Drought Experiment (ambient plots in black, drought plots in brown, non-parametric two-sample Wilcoxon test, ns: *p* > 0.05, $*$: *p* ? 0.05, $**$: *p* ? 0.01, $***$: *p* ? 0.001)."
scide_gg
rm(list = ls())
```
...
```{r exp-extra-lme-tab}
source("scripts/test-exp-lme.R")
knitr::kable(extra_lme, caption = "Comparison of Community Temperature Index (CTI, °C) and Community Precipitation Index (CPI, mm) in an additional treatment of the Jasper Ridge Global Change Experiment (JRGCE)s and two additional experiments, the McLaughlin Water Experiment (MWE) and the Santa Cruz International Drought Experiment (SCIDE). Pooling data from all years, we report the average difference in the metric between treatment and ambient plots, *p*-value, and significance level from linear mixed-effect model with a random intercept for each year (ns: *p* > 0.05, $.$: *p* ? 0.1, $*$: *p* ? 0.05, $**$: *p* ? 0.01, $***$: *p* ? 0.001). ")
rm(list = ls())
```
...
```{r rank-obs-fig, fig.width=10, fig.height=8}
#| fig.cap = "Rank abundance curves of species at the observational sites. Colors indicate the year of observation."
source("scripts/plot-rank.R")
obs_rank_gg
```

```{r rank-exp-fig, fig.width=12, fig.height=8}
#| fig.cap = "Rank abundance curves of species in the Jasper Ridge Global Change Experiment, comparing ambient treatments (black) and warming treatments (red). Lines show median values; ribbons show 95% intervals."
exp_rank_gg
rm(list = ls())
```
