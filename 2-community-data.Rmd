...
write_csv(elkhorn_tbl, str_c(.path$com_tidy, "elkhorn.csv"))
```
...
write_csv(swanton_tbl, str_c(.path$com_tidy, "swanton.csv"))
```
...
write_csv(ucsc_tbl, str_c(.path$com_tidy, "ucsc.csv"))
rm(list = ls())
```
...
Prepare a species list from all tidy data. Send the list to Josie and Justin for manual correction.

```{r tidy-spp, eval=FALSE}
source("scripts/compile-species.R")
rm(tidy_tax_tbl)
```

After Josie and Justin manually reviewed, read the misspelling table and rename species. 

```{r rename-spp}
source("scripts/rename-species.R")
```

Finally, save tidy data with corrected species names.

```{r save}
write_rds(final_exp_tbl, .path$com_exp)
write_rds(final_obs_tbl, .path$com_obs)
rm(list = ls())
```
