...
CFP spans large geographic (`r cfp_bbox["xmax"] %>% abs() %>% round(0)`-`r cfp_bbox["xmin"] %>% abs() %>% round(0)`° W longitude, `r cfp_bbox["ymin"] %>% round(0)`-`r cfp_bbox["ymax"] %>% round(0)`° N latitude) and climatic gradients (`r chelsa_cfp_df$t_min %>% signif(digits = 3)`-`r chelsa_cfp_df$t_max %>% signif(digits = 3)` °C temperature, `r chelsa_cfp_df$p_min %>% signif(digits = 3)`-`r chelsa_cfp_df$p_max %>% signif(digits = 3)` mm precipitation).
...
_Danthonia californica_ had `r n_occ_cool` records, with temperature of `r niche_tbl %>% filter(species_type=="cool") %>% pull(tmp_occ_median) %>% signif(3) %>% format(big.mark = ",")` (`r niche_tbl %>% filter(species_type=="cool") %>% pull(tmp_occ_q05) %>% signif(3) %>% format(big.mark = ",")` -`r niche_tbl %>% filter(species_type=="cool") %>% pull(tmp_occ_q95) %>% signif(3) %>% format(big.mark = ",")` )°C and precipitation of `r niche_tbl %>% filter(species_type=="cool") %>% pull(ppt_occ_median) %>% signif(3) %>% format(big.mark = ",")` (`r niche_tbl %>% filter(species_type=="cool") %>% pull(ppt_occ_q05) %>% signif(3) %>% format(big.mark = ",")`-`r niche_tbl %>% filter(species_type=="cool") %>% pull(ppt_occ_q95) %>% signif(3) %>% format(big.mark = ",")`) mm, whereas _Stipa pulchra_ had `r n_occ_warm` records, with a median temperature of `r niche_tbl %>% filter(species_type=="warm") %>% pull(tmp_occ_median) %>% signif(3) %>% format(big.mark = ",")` (`r niche_tbl %>% filter(species_type=="warm") %>% pull(tmp_occ_q05) %>% signif(3) %>% format(big.mark = ",")`-`r niche_tbl %>% filter(species_type=="warm") %>% pull(tmp_occ_q95) %>% signif(3) %>% format(big.mark = ",")`) °C and precipitation of `r niche_tbl %>% filter(species_type=="warm") %>% pull(ppt_occ_median) %>% signif(3) %>% format(big.mark = ",")` (`r niche_tbl %>% filter(species_type=="warm") %>% pull(ppt_occ_q05) %>% signif(3) %>% format(big.mark = ",")`-`r niche_tbl %>% filter(species_type=="warm") %>% pull(ppt_occ_q95) %>% signif(3) %>% format(big.mark = ",")`) mm.
...
These observational sites spanned an approximately `r site_hull_area %>% signif(1) %>% format(big.mark = ",")` km^2 geographical area.
...
Across the observations, species gained in communities by significantly increasing abundance over time (e.g., _`r obs_gainloss_eg1 %>% filter(change == "increase") %>% pull(species)`_) or recruitment (appeared only after the 5th yr, e.g., _`r obs_gainloss_eg2 %>% filter(complete_change == "new") %>% pull(species)`_); in contrast, species lost from communities by significantly decreasing abundance over time (e.g., _`r obs_gainloss_eg1 %>% filter(change == "decrease") %>% pull(species)`_) or extirpation (disappeared in the last 5 yr, e.g., _`r obs_gainloss_eg2 %>% filter(complete_change == "lost") %>% pull(species)`_). In the experiment, species did not experience complete turnover, but some gained by being more abundant (e.g., _`r exp_gainloss_eg %>% filter(change == "increase") %>% pull(species)`_), while some lost by being less abundant (e.g., _`r exp_gainloss_eg %>% filter(change == "decrease") %>% pull(species)`_) in warming compared to ambient plots.
