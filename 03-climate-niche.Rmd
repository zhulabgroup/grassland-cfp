...
gbif_occ_box_list <-
  foreach(
    ...
    res <- occ(
      ...
      gbifopts = list( # occ_options(from = "gbif", where = "console")
...
write_rds(gbif_occ_df, "data/occurrence/gbif/all-spp-cfp-2022-04-25.rds")
```
...
occ_sf_list[["gbif"]] <- read_rds("data/occurrence/gbif/all-spp-cfp-2022-04-25.rds") %>%
...
Not querying the data separately because:
...
```{r}
occ_sf_list[["inat"]] <- read_rds("data/occurrence/gbif/all-spp-cfp-2022-04-25.rds") %>%
...
### BIEN
Retreive all BIEN data then select those within CFP.
```{r, eval=F}
library(foreach)
library(doSNOW)
num_cores <- 22 # socs-stats has 44 cores
...
```{r, eval=F}
cch_raw_df<-read_csv("data/occurrence/cch/occurrences.csv")

cch_all_df<-cch_raw_df%>% 
  filter(scientificName %in% spp_tbl$canonicalname) %>% 
  dplyr::select(key=id, latitude=decimalLatitude, longitude=decimalLongitude, queryName=scientificName) %>% 
  drop_na(latitude, longitude)

cch_all_sf <- st_as_sf(
  x = cch_all_df %>% dplyr::select(key,longitude, latitude),
  coords = c("longitude", "latitude"),
  crs = 4326
)
cch_cfp_sf <- st_intersection(cch_all_sf, cfp_sf)
cch_cfp_df <- cch_all_df %>%
  right_join(as_tibble(cch_cfp_sf) %>% dplyr::select(key), by = "key")
write_rds(cch_all_df, "data/occurrence/cch/cch-all-2022-04-27.rds")
write_rds(cch_cfp_df, "data/occurrence/cch/cch-cfp-2022-04-27.rds")
```
...
occ_sf %>% 
  as_tibble() %>% 
  group_by(dataset) %>% 
  summarise (record_number=n()%>% format(trim = TRUE, scientific = FALSE, big.mark = ","),
             species_number=unique(species) %>% length()%>% format(trim = TRUE, scientific = FALSE, big.mark = ","))
```

Make a map to show all occurrence locations (randomly sample 10% records to save render time).
...
ggplot(occ_clim_sf %>% 
         ...
         mutate(chelsa=case_when(var=="vpdmax"~chelsa/100,
                                 TRUE~chelsa))
         )+
...
```
```{r}
tmean_chelsa<-occ_clim_sf %>%as_tibble() %>% filter(dataset=="chelsa") %>% pull(tmean)
tmean_prism<-occ_clim_sf %>%as_tibble() %>% filter(dataset=="prism") %>% pull(tmean)
cor.test(tmean_chelsa,tmean_prism)

ppt_chelsa<-occ_clim_sf %>%as_tibble() %>% filter(dataset=="chelsa") %>% pull(ppt)
ppt_prism<-occ_clim_sf %>%as_tibble() %>% filter(dataset=="prism") %>% pull(ppt)
cor.test(ppt_chelsa,ppt_prism)

vpdmax_chelsa<-occ_clim_sf %>%as_tibble() %>% filter(dataset=="chelsa") %>% pull(vpdmax)
vpdmax_prism<-occ_clim_sf %>%as_tibble() %>% filter(dataset=="prism") %>% pull(vpdmax)
...
range_compare_clim_summary<-read_rds("data/climate/range_comparison_with_climate_data.rds") %>% 
  ...
  summarise(q5=quantile(value, 0.05, na.rm=T),
