...
write_rds(gbif_occ_df, "data/occurrence/gbif/all-spp-cfp-2022-03-23.rds")
```

Read GBIF occurrence data as a sf object. 

```{r}
occ_sf <- read_rds("data/occurrence/gbif/all-spp-cfp-2022-03-23.rds") %>%
  st_as_sf(coords = c("longitude", "latitude"), crs = 4326) # WGS84
```

Make a map to show all occurrence locations (randomly sample 10% to save render time).
...
Download and process [CHELSA climatology data v2.1](https://chelsa-climate.org/downloads/). We used annual data of temperature and precipitation (see metadata below). Daily and monthly data are also available.
...
```{r}
chelsa_path <- "data/climate/chelsa/"
tmean_ras <- raster::raster(paste0(chelsa_path, "bio1.tif"))
ppt_ras <- raster::raster(paste0(chelsa_path, "bio12.tif"))
vpdmax_ras <- raster::raster(paste0(chelsa_path, "vpd_max.tif"))
...
```{r}
write_rds(niche_tbl, "data/climate/niche-estimates-cfp.rds")
```
