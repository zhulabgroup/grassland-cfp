...
niche_tbl <- read_rds("data/climate/climate-gbif-2022-05-10.rds") %>%
  ...
  summarize(
    ...
    tmp_occ_sd = sd(tmp, na.rm = TRUE),
    ppt_occ_mean = mean(ppt, na.rm = TRUE),
    ppt_occ_logmean = log(ppt) %>% mean(na.rm = TRUE) %>% exp(), # ppt distn is lognormal
...
```{r}
niche_tbl %>%
  select(species, occ_n, tmp_occ_mean, tmp_occ_sd, ppt_occ_mean, ppt_occ_logmean, ppt_occ_sd, vpd_occ_mean, vpd_occ_sd) %>%
