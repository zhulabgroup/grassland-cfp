...
spp_consol_tbl <- read_csv("data/occurrence/species-consolidation-2022-06-08.csv", col_types = "c")

spp_vec <- c(spp_comm_tbl$species, spp_consol_tbl$query_name) %>%
  unique() %>%
  sort()
```
...
write_rds(gbif_cfp_tbl, "data/occurrence/gbif/gbif-cfp-2022-06-08.rds")

rm(cl, num_cores, gbif_box_ls, gbif_box_tbl, gbif_cfp_tbl)
```

Note that GBIF depends on download dates. Here's a test.
Yiluan's notes: I compared my query today with queries on Mar 23 and Apr 18. I noticed that differences between today's query and Mar 23 query are from crawling events that took place after Mar 23. Some records were removed, many were added. The records are the same between today and Apr 18. I noticed there were some new crawling events after Apr 18, but they did not change the content of the records.
...
```{r, eval=FALSE}
read_rds("data/occurrence/gbif/gbif-cfp-2022-05-10.rds") %>%
  filter(datasetName == "iNaturalist research-grade observations") %>%
  write_rds("data/occurrence/inat/inat-cfp-2022-05-10.rds")
```
...
cch_all_tbl <- read_csv("data/occurrence/cch/occurrences.csv") %>%
  filter(scientificName %in% spp_comm_tbl$species) %>%
...
bien_tbl <- read_rds("data/occurrence/bien/bien-cfp-2022-04-27.rds") %>%
  ...
  filter(species %in% spp_comm_tbl$species) %>%
...
```{r, eval=FALSE}
gbif_sf_wgs84 <- read_rds("data/occurrence/gbif/gbif-cfp-2022-05-10.rds") %>%
  mutate(
    species = queryName,
    key = as.character(key)
  ) %>%
...
bind_cols(
  ...
  write_rds("data/climate/climate-gbif-2022-05-28.rds")
```
...
```{r, eval=FALSE}
clim_long_sf <- read_rds("data/climate/climate-gbif-2022-05-28.rds") %>%
  pivot_longer(
...
```{r, eval=FALSE}
# runtime ~= 4 min
pdf("figures/species-climate-niche-2022-05-10.pdf", width = 8, height = 8 * .618)
print(niche_gg)
dev.off()
```
...
```{r}
niche_tbl <- read_rds("data/climate/climate-gbif-2022-05-10.rds") %>%
...
niche_tbl %>%
  ggplot(aes(
    ...
    x = tmp_occ_median,
    y = ppt_occ_median,
    xmin = tmp_occ_q25, # tmp_occ_mean - tmp_occ_sd / sqrt(occ_n),
    xmax = tmp_occ_q75, # tmp_occ_mean + tmp_occ_sd / sqrt(occ_n),
    ymin = ppt_occ_q25, # ppt_occ_mean - ppt_occ_sd / sqrt(occ_n),
    ymax = ppt_occ_q75 # ppt_occ_mean + ppt_occ_sd / sqrt(occ_n)
  )) +
  geom_point() +
  geom_errorbarh(alpha = .05) +
  geom_errorbar(alpha = .05) +
  labs(
...
niche_tbl %>%
  ...
  write_rds("data/occurrence/niche-estimates-cfp-2022-05-10.rds")
```
