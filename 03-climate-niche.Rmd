...
```{r, eval=FALSE}
read_rds("data/occurrence/gbif/gbif-cfp-2022-06-08.rds") %>%
  left_join(spp_consol_tbl, by = c("queryName" = "query_name")) %>%
  select(consolidated_name, everything()) %>%
  rename(consolidatedName = consolidated_name) %>%
  mutate(
    consolidatedName =
      ifelse(
  ...
  write_rds("data/occurrence/gbif/gbif-consolidated-2022-06-08.rds")
```
