...
gbif_tbl <- read_rds("data/occurrence/gbif/gbif-cfp-2022-05-02.rds") %>%
  mutate(
    dataset = "gbif",
    species = queryName,
    key = as.character(key)
...
inat_tbl <- read_rds("data/occurrence/inat/inat-cfp-2022-05-02.rds") %>%
  mutate(
    dataset = "inat",
    # species = queryName,
    key = as.character(key)
...
```{r, eval=FALSE}
gbif_sf_wgs84 <- read_rds("data/occurrence/gbif/gbif-cfp-2022-05-02.rds") %>%
  mutate(
    species = queryName,
    key = as.character(key)
  ) %>%
...
bind_cols(
  ...
  write_rds("data/climate/climate-gbif-2022-05-02.rds")
```

Compare CHELSA and PRISM climate data at GBIF occurrence locations, by making scatter plots with correlation. 

```{r}
clim_gbif_tbl <- read_rds("data/climate/climate-gbif-2022-05-02.rds") %>%
  as_tibble() %>%
...
gbif_chelsa_sf <- read_rds("data/climate/climate-gbif-2022-05-02.rds") %>%
...
```{r, eval=FALSE}
# runtime ~= 4 min
pdf("figures/species-climate-niche-2022-05-03.pdf", width = 8, height = 8 * .618)
print(niche_gg)
dev.off()
```
