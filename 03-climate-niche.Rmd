...
spp_consol_tbl <- googlesheets4::read_sheet("1ez43lbFMsTJwmkW_23icDabt30ttDhVZ-1vK8Ts_Mck", sheet = "Consolidation")
...
spp_tbl <- spp_comm_tbl %>%
  ...
  filter(!str_detect(query_name, "AVERAGE")) %>% 
  arrange(query_name) %>% 
...
write_rds(gbif_cfp_tbl, "data/occurrence/gbif/gbif-cfp-2022-07-01.rds")
...
```{r, eval=FALSE}
read_rds("data/occurrence/gbif/gbif-cfp-2022-07-01.rds") %>%
  ...
  write_rds("data/occurrence/gbif/gbif-consolidated-2022-07-01.rds")
```

Pointer to file name.

```{r}
gbif_file <- "data/occurrence/gbif/gbif-consolidated-2022-07-01.rds"
```
...
read_rds(gbif_file) %>%
  ...
  write_rds("data/occurrence/inat/inat-cfp-2022-07-01.rds")
```

Pointer to file name.

```{r}
inat_file <- "data/occurrence/inat/inat-cfp-2022-07-01.rds"
```
...
write_rds(cch_all_tbl, "data/occurrence/cch/cch-all-2022-07-01.rds")
write_rds(cch_cfp_tbl, "data/occurrence/cch/cch-cfp-2022-07-01.rds")
...
```{r}
cch_file <- "data/occurrence/cch/cch-cfp-2022-07-01.rds"
```
...
Remove all occurrance data (but leave file name pointers).
...
bind_cols(
  ...
  write_rds("data/climate/climate-gbif-2022-07-01.rds")
```

Pointer to file name.

```{r}
clim_file <- "data/climate/climate-gbif-2022-07-01.rds"
```
...
```{r, eval=FALSE}
# runtime ~= 4 min
pdf("figures/species-climate-niche-2022-07-01.pdf", width = 8, height = 8 * .618)
print(niche_gg)
dev.off()
```
