...
```{r}
gbif_tib <- read_rds("data/occurrence/gbif/obs-spp-gbif.rds")
gbif_tib %>%
  select(queryName, name, longitude, latitude, locality) %>% 
  filter(queryName == "Achillea millefolium") %>%
  mapr::map_leaflet(size = 1)
```
