...
## File goal
For CTI or CPI to be valid measures, we want to see that the histograms of species-specific thermal and precipitation index values. This file will produce histograms for each species in the datasets we are focusing on, as well as help me isolate which species should be removed because there are too few datapoints to make a good temp/precip curve.

```{r OLD -- call all species, include = FALSE}
## This is the old way I called up all of the data. 
...
all_species <- all_species %>%
  ...
  dplyr::select(-all_species) %>% rename(all_species = all_species.2)
...
# Call data from GBIF #
all_species_data <- occ(query = all_species_list, from = "gbif", has_coords = TRUE, limit = 50000,
...
```{r extra species df}
extras <- c("Briza minor",
"Castilleja attenuata",
"Anagallis arvensis")


extras_data <- occ(query = extras, from = "gbif", has_coords = TRUE, limit = 50000,
                gbifopts = list(hasGeospatialIssue = FALSE, country = "US",
                                stateProvince = "California"))
...
## we can also replace all synonym taxon names with one (cuts down the number of plotted dot colors)
## we should use the scrubr version of the function in the future
extras_data <- fixnames(extras_data, how = "query")

## The occ2df function should convert the data to a dataframe format
extras_data_df <- occ2df(extras_data)
...
flags <- clean_coordinates(x = extras_data_df, lon = "longitude", lat = "latitude",
                          species = "name",
                          tests = c("capitals", "centroids", "equal","gbif", "institutions",
                                    "seas", "zeros", "duplicates")) # most tests are on by default
...
# Most of our flagged coordinates are coastal/in the ocean. Therefore, we'll remove them from our database.
# keep only the non-flagged data
extras_data_df <- extras_data_df[flags$.summary,] %>%
  rename("species.name" = "name")
...
Here are the histograms for the temperature and precipitation where these species have been recorded.
...
There are a handful of species for which we only have a few records, so we should consider removing those from the dataset. 

## Species with limited data

```{r small number species}
all_species_counts <- all_species_climate %>%
  group_by(species.name) %>%
  summarise(count = n())
```


Based on this, we should remove the following species from the dataset: 
Linanthus jepsonii, Senecio eurycephalus, Linanthus latisectus, Centaurium trichanthum, Navarretia jepsonii. 
