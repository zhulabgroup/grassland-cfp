...
rel_all_SLA_means <- rel_all_SLA %>%
  ...
  summarise(meanSLA = mean(meanSLA),
            cover_inc = sum(relcov)) %>%
  filter(cover_inc > 0.75) %>%
  group_by(site, year) %>%
  summarise(mean_SLA = mean(meanSLA),
            seSLA = std.error(meanSLA))

ggplot(rel_all_SLA_means, aes(x = year, y = mean_SLA, color = site)) +
  geom_point() +
  geom_line() +
  geom_errorbar(aes(ymin = (mean_SLA - seSLA), ymax = (mean_SLA + seSLA))) +
