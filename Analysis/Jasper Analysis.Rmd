...
We'll grab the climate data we downloaded from PRISM.

```{r monthly weather summaries}
jaspw_sum <- climate_site %>%
  filter(sites.site == "Jasper") %>%
  mutate(Year1 = year,
         Month1 = month) %>%
  unite(Year1, Month1, col = "date", sep = "-")

jaspw_sum$date <- lubridate::ymd(jaspw_sum$date, truncated = 1L)

jaspw_sum <- jaspw_sum %>%
  mutate(date1 = date,
         wateryr = wtr_yr(date1))

ggplot(jaspw_sum, aes(x = date, y = ppt)) +
...
ggplot(jaspw_year, aes(x = year, y = precip)) +
  geom_bar(stat = "identity") +
  labs(x = "Date", y = "Precipitation (mm)")

jaspw_wint <- jaspw_sum %>%
  filter(month == "11" | month == "12" | month == "01" | month == "02" ) %>%
  group_by(wateryr) %>%
  summarise(precip = sum(ppt),
            tmean = mean(tmean)) %>%
  rename(year = wateryr) 

ggplot(jaspw_wint, aes(x = year, y = precip)) +
  geom_bar(stat = "identity") +
  labs(x = "Date", y = "Precipitation (mm)")
...
ggplot(jasp.changerate, aes(interval, distance, color = trtrep)) + 
