...
species_list_ang <- c('Danthonia californica', 
                      'Bromus hordeaceus',
                      'Bromus tectorum',
                      'Bromus diandrus',
                      'Aira caryophyllea',
...
write_rds(ang_species_data_df, "Analysis/GBIF_Data/AngGBIF.rds")
```
...
write.csv(ang_CTI, "Analysis/CTI Data/Ang_CTI_07_2020.csv")
```
...
# and we'll save the angelo gbif spatial data
write_rds(car_species_data_df, "Analysis/GBIF_Data/CarGBIF.rds")
```

```{r remove carrizo objects}
remove(car_allium_data_df)
remove(car_astragalus_data_df)
remove(car_delpinium_data_df)
remove(car_species)
```
...
write.csv(car_CTI, "Analysis/CTI Data/Car_CTI_07_2020.csv")
```
...
# and we'll save the angelo gbif spatial data
write_rds(elk_species_data_df, "Analysis/GBIF_Data/ElkGBIF.rds")
```


```{r remove carrizo objects}
remove(elk_bromus_data_df)
remove(elk_brassica_data_df)
remove(elk_species)
```
...
write.csv(elk_CTI, "Analysis/CTI Data/Elk_CTI_07_2020.csv")
```
...
# and we'll save the angelo gbif spatial data
write_rds(jasp_species_data_df, "Analysis/GBIF_Data/JaspGBIF.rds")
```
...
```{r mean jasp species temp, echo=FALSE}
...
write.csv(jasp_CTI, "Analysis/CTI Data/Jasp_CTI_07_2020.csv")
```
...
# and we'll save the angelo gbif spatial data
write_rds(mcLann_species_data_df, "Analysis/GBIF_Data/mcLannGBIF.rds")
```
...
```{r mcLann mean species temp, echo=FALSE}
...
write.csv(mcLann_CTI, "Analysis/CTI Data/mcLann_CTI_07_2020.csv")
```
...
mcLserp_species_data <- occ(query = mcLann_species_list, from = "gbif", has_coords = TRUE, limit = 10000,
                gbifopts = list(hasGeospatialIssue = FALSE, country = "US",
                                stateProvince = "California"))
```
...
# and we'll save the mclserp gbif spatial data
write_rds(mcLserp_species_data_df, "Analysis/GBIF_Data/mcLserpGBIF.rds")
...
```{r mclserp mean species temp, echo=FALSE}
...
write.csv(mcLserp_CTI, "Analysis/CTI Data/mcLserp_CTI_07_2020.csv")
```
...
rel_swan <- rel_all %>%
  filter(site == "Swanton")
swan_species <- unique(rel_mcLserp$species.name)
...
# and we'll save the swanton gbif spatial data
write_rds(swan_species_data_df, "Analysis/GBIF_Data/swanGBIF.rds")
```
...
```{r swanton mean species temp, echo=FALSE}
...
write.csv(swan_CTI, "Analysis/CTI Data/swan_CTI_07_2020.csv")
```
...
# and we'll save the swanton gbif spatial data
write_rds(ucsc_species_data_df, "Analysis/GBIF_Data/ucscGBIF.rds")
```
...
write.csv(ucsc_CTI, "Analysis/CTI Data/ucsc_CTI_07_2020.csv")
```