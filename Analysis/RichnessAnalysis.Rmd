...
ang_grich <- angelo_data %>%
  ...
  ungroup() %>% 
  gather('Nat', 'Exo', 'NatAnns', 'ExoAnns', 'ExoPers', 'NatPers', key = "guild", value = "rich") %>%
  group_by(year, guild) %>%
...
grid.arrange(
  ggplot(nat.grich, aes(x = year, y = meanrich, color = site)) +
  geom_point() +
    geom_line() +
  labs(title = "",
       x = "Year",
  ...
  ggplot(exo.grich, aes(x = year, y = meanrich, color = site)) +
  geom_point() +
    geom_line() +
  labs(title = "",
       x = "Year",
