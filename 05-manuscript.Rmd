...
clim_file <- "data/climate/climate-gbif-2022-07-04.rds"
...
cch_file <- "data/occurrence/cch/cch-cfp-2022-07-04.rds"
gbif_file <- "data/occurrence/gbif/gbif-consolidated-2022-07-04.rds"
inat_file <- "data/occurrence/inat/inat-cfp-2022-07-04.rds"

cfp_file <- "data/occurrence/hotspots/hotspots_2016_1.shp"
site_file <- "data/community/site-info.rds"
```
...
```{r}
rm(clim_niche_gg, gbif_chelsa_sf, niche_gg, occ_clim_gg, occ_geog_gg)
```
...
obs_gg_fun <- function(tbl, site_name, cti_lab = "", cpi_lab = "", yr_lab = NULL) {
  ...
  ggplot(site_tbl, aes(year, com_idx_value, group = year)) +
    geom_boxplot() +
    geom_smooth(
...
angelo_gg <- obs_gg_fun(obs_idx_tbl, "angelo", cti_lab = "CTI (°C)", cpi_lab = "CPI (mm)")
carrizo_gg <- obs_gg_fun(obs_idx_tbl, "carrizo")
elkhorn_gg <- obs_gg_fun(obs_idx_tbl, "elkhorn")
jasper_gg <- obs_gg_fun(obs_idx_tbl, "jasper")
mclann_gg <- obs_gg_fun(obs_idx_tbl, "mclann", cti_lab = "CTI (°C)", cpi_lab = "CPI (mm)")
mclserp_gg <- obs_gg_fun(obs_idx_tbl, "mclserp")
pleasantonridge_gg <- obs_gg_fun(obs_idx_tbl, "pleasantonridge")
sunol_gg <- obs_gg_fun(obs_idx_tbl, "sunol")
swanton_gg <- obs_gg_fun(obs_idx_tbl, "swanton", cti_lab = "CTI (°C)", cpi_lab = "CPI (mm)")
ucsc_gg <- obs_gg_fun(obs_idx_tbl, "ucsc")
vascocaves_gg <- obs_gg_fun(obs_idx_tbl, "vascocaves")
```
...
```{r}
obs_gg <-
  angelo_gg + carrizo_gg + elkhorn_gg + jasper_gg +
  mclann_gg + mclserp_gg + pleasantonridge_gg + sunol_gg +
  swanton_gg + ucsc_gg + vascocaves_gg +
  plot_annotation(tag_levels = "A") +
  plot_layout(design = "
  ABCD
  EFGH
...
```{r, fig.width=10, fig.height=10*1.618, fig.cap="Grassland community shifts from long-term observations. (A to K) Consistent with climate warming and drying, grassland communities shift dominance to warmer (CTI) and drier (CPI) species in 11 long-term observational sites across the California Floristic Province (dashed trend lines by linear regression, all significant slopes by *t*-test, *p* < 0.05)."}
obs_gg
```
...
```{r}
grass_path <- "data/occurrence/grassland/"
sds <- terra::sds(paste0(grass_path, "MCD12C1.A2020001.006.2021362215328.hdf"))
