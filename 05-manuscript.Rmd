...
```{r}
gbif_chelsa_sf <- read_rds("data/climate/climate-gbif-2022-05-28.rds") %>%
...
dir.create(grass_path, recursive = T)
library(terra)
sds<-sds(paste0(grass_path, "MCD12C1.A2020001.006.2021362215328.hdf"))
lc_ras<-raster(sds[1])
extent(lc_ras)<-c(-180, 180,-90, 90)
projection(lc_ras) <- CRS("+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0")
lc_ras<-flip(lc_ras)
lc_ras <- crop(lc_ras, extent(cfp_sf))
lc_ras <- mask(lc_ras, cfp_sf)
...
grass_df<-as.data.frame(grass_ras, xy=T) %>% 
  drop_na()
```
