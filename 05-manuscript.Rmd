...
obs_gg_fun <- function(tbl, site_lab, cti_lab = "", cpi_lab = "", yr_lab = NULL) {
  ggplot(data = tbl, aes(year, com_idx_value, group = year)) +
    ...
    expand_limits(x = range(obs_tbl$year)) +
    labs(
      x = yr_lab, y = NULL,
      title = site_lab
    ) +
...
mclserp_gg <- obs_com_mean_tbl %>%
  filter(site == "mclserp") %>%
  obs_gg_fun(site_vec["mclserp"])

pleasantonridge_gg <- obs_com_mean_tbl %>%
  filter(site == "pleasantonridge") %>%
  obs_gg_fun(site_vec["pleasantonridge"])

sunol_gg <- obs_com_mean_tbl %>%
  filter(site == "sunol") %>%
  obs_gg_fun(site_vec["sunol"])

swanton_gg <- obs_com_mean_tbl %>%
  filter(site == "swanton") %>%
  obs_gg_fun(site_vec["swanton"],
    cti_lab = "CTI (°C)", cpi_lab = "CPI (mm)"
  )
...
vascocaves_gg <- obs_com_mean_tbl %>%
  filter(site == "vascocaves") %>%
  obs_gg_fun(site_vec["vascocaves"])
```
