...
angelo_gg <- obs_com_idx_tbl %>%
  filter(site == "angelo") %>%
  obs_gg_fun(c(angelo = "Angelo Coast Range Reserve"),
    cti_lab = "CTI (°C)", cpi_lab = "CPI (mm)"
  )

carrizo_gg <- obs_com_idx_tbl %>%
  filter(site == "carrizo") %>%
  obs_gg_fun(c(carrizo = "Carrizo Plain"))

elkhorn_gg <- obs_com_idx_tbl %>%
  filter(site == "elkhorn") %>%
  obs_gg_fun(c(elkhorn = "Elkhorn Slough"))

jasper_gg <- obs_com_idx_tbl %>%
  filter(site == "jasper") %>%
  obs_gg_fun(c(jasper = "Jasper Ridge Biological Preserve"),
    cti_lab = "CTI (°C)", cpi_lab = "CPI (mm)"
  )

mclann_gg <- obs_com_idx_tbl %>%
  filter(site == "mclann") %>%
  obs_gg_fun(c(mclann = "McLaughlin Reserve"),
    yr_lab = "Year"
  )

ucsc_gg <- obs_com_idx_tbl %>%
  filter(site == "ucsc") %>%
  obs_gg_fun(c(ucsc = "UC Santa Cruz"))
```
