...
# reshape data
obs_idx_tbl <- obs_tbl %>%
  dplyr::select(site, year, plot, tmp_com_mean, ppt_com_mean, cwd_com_mean) %>%
  ...
  out_gg <- ggplot(site_tbl, aes(year, com_idx_value)) +
    ...
    facet_wrap(~com_idx_name,
      ...
      labeller = labeller(com_idx_name = c(
        ...
        CDI = cdi_lab # "Community Drought Index\n(CDI, mm)"
      ))
    ) +
...
cwd_obs_landsc_gg <-
  ...
  plot_cwm(obs_idx_tbl, "mclserp") +
  plot_cwm(obs_idx_tbl, "morganterritory", cti_lab = "CTI (°C)", cpi_lab = "CPI (mm)", cdi_lab = "CDI (mm)", yr_axis = TRUE) +
