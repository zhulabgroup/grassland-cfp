...
for (param in param_list) {
  dir.create(paste0("./data/climate/chelsa_monthly/", param))
  system(paste0("wget --no-host-directories --no-directories --input-file=./data/climate/chelsa_monthly/", param, "_paths.txt --continue --directory-prefix=./data/climate/chelsa_monthly/", param, "/"))
}
...
for (param in param_list) {
  val_list <- foreach(
    ...
    files <- list.files(paste0("./data/climate/chelsa_monthly/", param), pattern = year %>% as.character(), full.names = T)
...
write_rds(val_df, file = "./data/climate/chelsa_monthly/annual.rds")
stopcluster(cl)
